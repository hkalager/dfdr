...
% Last Modified 09 September 2018 16:09 IRST
clear;
clc;
...
for i=1:numel(labels)
    ...
    for yearOOS=yearser(1):yearser(end)%2006:2016
        for month=1:12
            for IS=2
                ...
                resvar{iter,4}=pvalues_Mu;
                resvar{iter,5}=Perf;
                resvar{iter,6}=Perf_Mu;
                % Written to table
                resvar{iter,1+6}=yearOOS;
                resvar{iter,2+6}=sum(PORTFDR);
                resvar{iter,3+6}=sum(PORTFDR_Mu);
                resvar{iter,4+6}=(1+PortRetIS)^(1/IS)-1;
                resvar{iter,5+6}=(1+PortRetIS_Mu)^(1/IS)-1;
                resvar{iter,6+6}=(1+mean(PortRetOOS1M))^12-1;
                resvar{iter,7+6}=(1+mean(PortRetOOS1M_Mu))^12-1;
                resvar{iter,8+6}=regions{i};
                resvar{iter,9+6}=month;
                toc
    ...
    result_table=cell2table(resvar(:,7:end));
    result_table.Properties.VariableNames={'Year' ...
        'PortSizeSharpe' 'PortSizeMu' ...
        'FDRRetISSharpe' 'FDRRetISMu'  ...
        'FDRRetOOS1MSharpe' 'FDRRetOOS1MMu'  'Country' 'Month'};
    ...
    writetable(result_table,[drpadd,'Mu','_',lbl,'_',num2str(yearser(1)),'_',num2str(yearser(end)),'.xlsx'])
    save(['Mu','_',lbl,'_',num2str(yearser(1)),'_',num2str(yearser(end)),'.mat'],'resvar');
